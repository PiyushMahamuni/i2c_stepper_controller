...
public:
    ...
    /// Sets the acceleration/deceleration rate.
    /// \param[in] acceleration The desired acceleration in steps per seconds^2
    /// \param[in] sqrt_acceleration The square root of the \param[in] acceleration
    /// Must be > 0.0. This is an overloaded call where you provide both acceleratoin
    ...
    // /// Moves the motor (with acceleratin/deceleration)
    // /// to the absolute target position and blocks until it is at
    // /// position. Don't use this in event loops, since it blocks.
    // /// \param[in] position - The absolute target position.
    // void    runToAbsPosition(long position);
